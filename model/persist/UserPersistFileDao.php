...
class UserPersistFileDao {
    ...
    /**
     * Checks if a username is in a database
     * @param string username given by the user
     * @return User|null User object if found or null if not  
     */
    public function searchWhereUser( string $user): ?User{
        $resultObj = null;
        //get all data.
        $objList = $this->selectAll();
        for ($index = 0; $index < count($objList); $index++) {
            $elem = $objList[$index];
            if ($elem->getUsername()===$user) {
                $resultObj = $elem;  
                break;
            }
        }
    ...
    /**
     * Given an id return a user object if found or null
     * @param int id of user
     * @return object User if found or null if not
     */
     ...
     */
    public function arraySearchIndex(array $list, User $obj): int {
        $index = -1;
     ...
     */
    public function fromFieldsToObj(array $fields): ?User {
        $id =  intval($fields[0]);
        $username = $fields[1];
        $password = $fields[2];
        $role = $fields[3];
